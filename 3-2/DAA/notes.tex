...
Before we can get started with some algorithms, we must discuss linear programming. A linear programming is of the form:
...
Linear programming has a concept of \textbf{duality}.  Consider the example of minimizing
\[6x_1 + 4x_2 + 2x_3\]
subject to
\[4x_1 + 2x_2 + x_3 \geq 5\]
\[x_1 + x_2 \geq 5\]
...
\[2y_1 + y_2 + y_3 \leq 4\]
\[y_1 + y_3 \leq 2\]
We want to maximize the lower bound subject to these constraints,which gives us a new LP - maximize
\[5y_1 + 3y_2 + 4y_3\]
subject to
\[4y_1 + y_2 \leq 6\]
\[2y_1 + y_2 + y_3 \leq 4\]
...
\[\sum_{j=1}^m c_jx_j \geq \sum_{i=1}^m b_iy_i\]
...
\[W_{LP} \leq W(S^*)\]
The ILP problem is NP-Complete, and we cannot hope to solve it. However, solving the LP problem is in P. We can solve it in polynomial time using the simplex or the ellipsoid algorithm.
...
The vertex cover problem is a special case of the set cover proble. We can convert a vertex cover problem into a set cover problem by letting the ground set be the set of edfes, and a subset is created for each vertex $i$ containing edges incident to $i$.
