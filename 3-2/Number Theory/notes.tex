...
A consequence of Lemma 8.1 is that $\ord_m a | \phi(m)$. This allows us to prune the space of possible orders when trying to find the order of an integer. Instead of trying every single integer to check if it is the order, we only need to check divisors of $\phi(m)$.

\begin{exercise}
  Let $a \in \Z$ with $(a,m) = 1$. Let $i,j \in \N \cup \{0\}$. Then $a^i \equiv a^j \mod m$ if and only if $i \equiv j \mod \ord_m a$  .
\end{exercise}
\begin{solution}
  TODO, exercise (consider calling this an exercise?) 
\end{solution}

A consequence of the proposition above is that if $k = \ord_m a$, then $a,a^2, \cdots , a^k$ are incongruent modulo $m$.
