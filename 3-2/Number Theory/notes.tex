...
\begin{theorem}
  Let $p$ be a prime and let $d$ be a positive divisor of $p-1$. Then there are exactly $\phi(d)$ incongruent integers of order $d$ modulo $p$. 
\end{theorem}
\begin{proof}
  For every positive divisor $d$ of $p-1$, let:
  \[S_d = \{a : 1 \leq a \leq p-1, \ord_p a = d\}\]
  Let $\psi(d) = |S_d|$. Since 
  \[\{1,2,3,\cdots, p-1\} = \bigcup_{d|p-1} S_d\]
  we get
  \[p-1 =\sum_{d|p-1} \psi(d) \]
  We know that:
  \[\sum_{d|p-1} \phi(d) = p-1 = \sum_{d|p-1} \psi(d)\]
  We want to prove that $\psi(d) = \phi(d) \forall d$. It is enough to prove that $\psi(d) \leq \phi(d)$.

  Suppose $d$ is a divisor of $p-1$ with $\psi(d) \neq 0$. Let $a$ be an integer with $\ord_p a = d$. We know that
  \[a,a^2, \cdots, a^d\]
  ...
  We know that
  \[\ord_p a^i = \frac{d}{(i,d)}\]
  Thus $\ord_p a^i = d$ if and only if $i$ and $d$ are coprime. There are $\phi(d)$ such $i$. Hence, $|S_d| = \psi(d) = \phi(d)$.
\end{proof}

\begin{corollary}
  Let $p$ be a prime. There are $\phi(p-1)$ incongruent primitive roots  modulo $p$. 
\end{corollary}
\begin{proof}
  Note that this also follows from Corollary 8.4.1. To show that this follows from Theorem 8.8, we see that if a number is a primitive root, it's order is $d = p-1$. So, there must be $\phi(p-1)$ incongruent primitive roots.
\end{proof}
...
\begin{lemma}
 ...
 \begin{enumerate}
   \item If $b \equiv a \mod p$, then $\legendre{a}{p} = \legendre{b}{p}$
...
From this, if $p \nmid a$, and $a = p_1^{e_1} p_2^{e_2} \cdots p_k^{e_k}$, then:
\[\legendre{a}{p} = \left (\legendre{p_1}{p} \right)^{e_1} \cdots \left( \legendre{p_k}{p} \right)^{e_k}\]
...
\end{exercise}
\begin{solution}
  Suppose $p \equiv 1 \mod 8$. So,
  \[p = 8k+1, k \in N\]
  Look at the first $(p-1)/2 = 4k$ even integers:
  \[2,4,6,\cdots, 8k\]
  Now, 
  \[2 \cdot 4 \cdot 6 \cdots 8k = 2^{4k} (4k)!\]
  Moreover,
  \[2 \cdot 4 \cdot 6 \cdots 4k \cdot (4k+2) \cdot (4k+4) \cdots (8k) \]
  \[\equiv 2 \cdot 4 \cdot 6 \cdots 4k \cdot (-4k+1) \cdot (-4k+3) \cdots (-3)(-1) \mod p\]
  \[\equiv (-1)^{2k} 2 \cdot 4 \cdot 6 \cdots 4k \cdot (4k-1) \cdots 3 \cdot 1 \mod p\]
  \[\equiv (4k)! \mod p\]
Hence, 
\[2^{4k} \equiv 1 \mod p\]. This implies that
\[\legendre{2}{p} = 1\]
Now, let $p = 8k+3$. Then, we would look at the first $(p-1)/2 = 4k+1$ even integers. By the same arguments, we find that
\[2^{4k} \equiv -1 \mod p\]
and hence
\[\legendre{2}{p} = -1\]
\end{solution}
...
\begin{definition}
  Let $p$ be an odd prime. Then, every integer coprime to $p$ is congruent modulo $p$ to exactly one integer in the list:
  \[-\frac{p-1}{2}, \cdots , -1, -1, 1, 2 , \cdots \frac{p-1}{2}\]
  The set of these integers is called the symmetric set of residues modulo $p$.
\end{definition}

\begin{definition}
  Let $a \in \Z$ with $p \nmid a$. The unique integer $s$ in the symmetric set of residues congruent  to $a$ is called the symmetric reduction of $a \mod p$.
\end{definition}

  The symmetric reductions of:
  \[a, 2a, \cdots \frac{p-1}{2} a\]
  help determine whether $a$ is a quadratic residue modulo $p$ or not. Let $\nu_a$ be the number of integers $k \in [1,(p-1)/2]$ such that the symmetric reduction of $ka$ is negative. Then, we get the following theorem.
...
\begin{theorem}[Quadratic Reciprocity Law]
  ...
  Equivalently,
  \[\legendre{q}{p} = \begin{cases}
    \legendre{p}{q} & \text{if } p \equiv 1 \mod 4 \text{ or } q \equiv 1 \mod 4  \\
    -\legendre{p}{q} & \text{if } p \equiv q \equiv 3 \mod 4
  \end{cases}\]
\end{theorem}
\begin{proof}
  Consider the rectangle with vertices $(0,0), (p/2,0), (p/2,q/2),(q/2,0)$. It's diagonal will have the equation $y = \frac{q}{p}x$.

  Let the number of lattice points in the rectangle be $N$. Then,
  \[N = \frac{p-1}{2} \cdot \frac{q-1}{2}\]
  Also, we can count it by the number of lattice points in the triangle and the number of lattice points on the diagonal. From the geometric interpretation of lemma 9.6, the sum of points in the triangles will be:
  \[\sum_{k=1}^{(p-1)/2} \left \lfloor \frac{kq}{p} \right \rfloor + \sum_{l=1}^{(p-1)/2} \left \lfloor \frac{lp}{q} \right \rfloor \]
  The number of lattice points on the diagonal is 0. Assume that a lattice point $(a,b)$ is on the diagonal. Then, $pb = qa$, or $p | a$. However, $a \in [1,(p-1)/2]$, which is a contradiction. Hence.
  \[\frac{p-1}{2} \cdot \frac{q-1}{2} = \sum_{k=1}^{(p-1)/2} \left \lfloor \frac{kq}{p} \right \rfloor + \sum_{l=1}^{(p-1)/2} \left \lfloor \frac{lp}{q} \right \rfloor\]
  From Gauss' Lemma, 
  \[\legendre{p}{q} \legendre{q}{p} = (-1)^{\sum_{k=1}^{(p-1)/2} \left \lfloor \frac{kq}{p} \right \rfloor} \cdot (-1)^{\sum_{l=1}^{(p-1)/2} \left \lfloor \frac{lp}{q} \right \rfloor }\]
  Hence,
  \[\legendre{p}{q} \legendre{q}{p} = (-1)^{\frac{p-1}{2} \frac{q-1}{2}}\]
\end{proof}

\begin{exercise}
  Determine all numbers $a \in [1,13]$ that are quadratic residues modulo 101. 
\end{exercise}
\begin{solution}
  Since $\legendre{a^2}{p} = 1$ for all primes $p$ and for every $a \in \Z$, we have:
  \[\legendre{1}{101} = \legendre{4}{101} = \legendre{9}{101} = 1\]
  Since $101 \equiv 5 \mod 8$, we have $\legendre{2}{101} = -1$. We can check using Gauss' Lemma that $\legendre{3}{101} = -1$. Using the Quadratic Reciprocity law, 
  \[\legendre{5}{101} = \legendre{101}{5} = \legendre{1}{5} = 1\]
  Similarly,
  \[\legendre{7}{101} = \legendre{101}{7} = \legendre{3}{7} = -1\]
  All the leftover numbers, can be done similarly.
\end{solution}

\begin{exercise}
  Determine if the following congruence is solvable:
  \[3x^2 + 6x + 5 = 0 \mod 73\]
\end{exercise}
\begin{solution}
  $ax^2 + bx^2 + c \equiv 0 \mod p$ has a solution if and only if $y^2 \equiv b^2 - 4ac \mod p$ has a solution, i.e.
  \[\legendre{b^2-4ac}{p} = 1\]
  In this case, $a=3,b=6,c=5$
  \begin{align*}
    \legendre{-24}{73} &= \legendre{-1}{73} \legendre{4}{73} \legendre{2}{73} \legendre{3}{73} \\
...
\begin{definition}
  Let $m \in N$ be odd and let $a \in Z$ with $(a,m) = 1$. If 
  \[m = p_1^{e_1} \cdots p_k^{e_k}\]
  is the prime factorization of $m$, we define
  \[\legendre{a}{m} = \prod_{i=1}^k \left ( \legendre{a}{p_i}\right)^{e_i}\]
  to be the Jacobi Symbol
\end{definition}

Essentially, the Jacobi symbol is a generalization of the Legendre symbol from primes to all odd numbers.

\begin{lemma}
  Let $m \in \N$   be odd and let $a \in \Z$ be such that $(a,m) = 1$. Then, if $x^2 \equiv a \mod m$ is solvable, $\legendre{a}{m} = 1$.
\end{lemma}
\begin{proof}
  If $x^2 \equiv a \mod m$ is solvable, then $x_2 \equiv a \mod p_i$ is solvable for all $i$. Hence, 
  \[\legendre{a}{m} = \prod \left( \legendre{a}{p_i} \right)^{e_i} = 1\]
\end{proof}
Notice that the converse to this lemma is not true - we can show that $x^2 \equiv 2 \mod 15$ is not solvable, but $\legendre{2}{15} = 1$.

The Jacobi symbol also has the following properties:
\begin{enumerate}
  \item If $b \equiv a \mod m$, then $\legendre{a}{m} = \legendre{b}{m}$
  \item $\legendre{a}{m} \legendre{b}{m} = \legendre{ab}{m}$
  \item $\legendre{a^2}{m} = 1$
\end{enumerate}

\begin{lemma}
  If $m$ is an odd number,
  \[\legendre{-1}{m} = (-1)^{(m-1)/2}\] 
\end{lemma}
\begin{proof}
  TODO
\end{proof}

\begin{theorem}[Generalized Quadratic Reciprocity Law]
  Let $m,n$ be coprime odd natural numbers. Then,
  \[\legendre{m}{n} \legendre{n}{m}  = (-1)^{\frac{m-1}{2} \cdot \frac{n-1}{2}}\]
\end{theorem}
\begin{proof}
  Suppose $m = \prod_i^k p_i^{e_i}$ and $n = \prod_i^l q_i^{d_i}$. Then,
  \[\legendre{m}{n} = \prod_{i=1}^l \legendre{m}{q_i}^{d_i} = \prod_{i=1}^l \prod_{j=1}^k \legendre{p_j}{q_i}^{e_jd_i}\]
  In the same way,
  \[\legendre{m}{n} = \prod_{j=1}^k \prod_{i=1}^l \legendre{q_i}{p_j}^{d_ie_j}\]
  Hence, 
  \begin{align*}
    \legendre{m}{n} \legendre{n}{m} &= \prod_{i=1}^l \prod_{j=1}^k \left( \legendre{p_j}{q_i}  \legendre{q_i}{p_j} \right)^{d_ie_j} \\
                                    &= \prod_{i=1}^l \prod_{j=1}^k ((-1)^{(p_j-1)(q_i-1)/4})^{d_ie_j} \\
                                    &= (-1)^{\sum d_i \frac{q_i-1}{2} \cdot \sum_j e_j \frac{p_j-1}{2}}
  \end{align*}
  TODO rest.
\end{proof}

\chapter{Finite Continued Fractions}

\begin{definition}
  A finite continued fraction is an expression of the form:
  \[a_0 + \cfrac{1}{a_1 + \cfrac{1}{a_2 + \cfrac{1}{\ddots \cfrac{1}{a_{n-1} + \frac{1}{a_n}}}}}\]
  where $a_i > 0 \forall i > 0$. The numbers $a_0, \cdots, a_n$ are called the partial quotients of the fraction. It is denoted by $[a_0; a_1, a_2, \cdots a_n]$
\end{definition}

If all the partial quotients are integers, then the fraction is said to be simple.

\begin{theorem}
  The value of any finite simple continued fraction is a rational number. 
\end{theorem}
\begin{proof}
  Let $[a_0; a_1, \cdots, a_n]$ be a finite simple continued fraction. We can prove this by induction on $n$. It is a trivial result for $n=1$. Assume that the result holds for $n=k$. Then,
  \[[a_0; a_1, \cdots, a_{k+1}] = a_0 + \frac{1}{[a_1; a_2, \cdots, a_{k}]}\]
  By our hypothesis, the denominator is a rational number. Therefore, $[a_0 ; a_1, \cdots, a_{k+1}]$ is also a rational number. By PMI, the result is true for all $n$.
\end{proof}

The converse is also true - we can express any rational number as a finite simple continued fraction. 

\begin{theorem}
  Any rational number can be written as a finite simple continued fraction.
\end{theorem}
\begin{proof}
  Let the number be $\frac{a}{b}$, where $b > 0$. By the Euclidean algorithm,
  \[a = ba_0 + r_1, 0 < r_1 < b\]
  ...
  \begin{align*}
    \frac{a}{b} &= a_0 + \frac{r_1}{b} \\
                &= a_0 + \cfrac{1}{\frac{b}{r_1}} \\
                &= a_0 + \cfrac{1}{a_1 + \frac{r_2}{r_1}} \\
                &= a_0 + \cfrac{1}{a_1+\cfrac{1}{a_2+\frac{r_3}{r_2}}}\\
                &\cdots\\
                &= a_0 + \cfrac{1}{a_1 + \cfrac{1}{a_2 +\cfrac{\ddots}{\cfrac{1}{a_{n-1} + \frac{1}{a_n}}}}}
  \end{align*}
\end{proof}
...
\begin{definition}
  The continued fractoin obtained from $[a_0;a_1, \cdots, a_n]$ by truncating after the $k^{th}$ partial denominator $a_k$ is called the $k^{th}$ convergent of the continued fraction and is denoted by $C_k$, i.e.
...
\begin{itemize}
  \item $C_0 = a_0$
  \item $C_n =[a_0;a_1, \cdots, a_n]$
  \item If $k < n$ then 
    \[ \left[a_0; a_1, \cdots, a_{k-1}, a_k + \frac{1}{a_{k+1}} \right] = C_{k+1}\]
\end{itemize}

\begin{theorem}
  The $k^{th}$ convergent of the finite continued fraction $[a_0; a_1, \cdots, a_n]$ is 
  \[C_k = \frac{p_k}{q+k}\]
  where
  \[p_0 = a_0, q_0 = 1\]
...
\end{theorem}
\begin{proof}
  We can prove this by induction on $k$. It is trivial to show that the result shows for $k \leq 2$. Assume that the result holds for all $k=m < n$  So,
  \[C_m = \frac{p_m}{q_m} = \frac{a_m p_{m-1} + p_{m-2}}{a_m q_{m-1} + q_{m-2}}\]
  Note that $p_{m-1}, p_{m-2}, q_{m-1}, q_{m-2}$ don't depend on $q_m$. Replacing $a_m$ by $a_m + \frac{a_m}{a_{m+1}}$, we get:
  \[\left[ a_0; a_1, \cdots, a_{m-1}, a_m + \frac{q}{a_{m+1}}\right] = \frac{\left(a_m + \frac{1}{a_{m+1}}\right)p_{m-1} + p_{m-2}}{\left(a_m + \frac{1}{a_{m+1}}\right)q_{m-1} + q_{m-2}}\]
  \begin{align*}
    C_{m+1} &= \frac{a_{m+1}(a_mp_{m-1} + p_{m-2})+p_{m-1}}{a_{m+1}(a_mq_{m-1} + q_{m-2})} \\
            &= \frac{a_{m+1}p_m + p_{m-1}}{a_{m+1}q_m + q_{m-1}}
  \end{align*}
  By PMI, we have proved this result true.
\end{proof}
