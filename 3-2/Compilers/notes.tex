...
Now let us look at our first bottom up parser, the \textbf{LR(0) parser}. Before we discuss the exact algorithm we must understand some concepts. First is the augmentation of grammar. If $G$ is a grammar with a start symbol $S$, the augmented grammar $G'$ which has a new start symbol $S'$ and a additional production:
\[S' \rightarrow S\]
...
\[A \rightarrow XY.Z\]
\[A \rightarrow XYZ.\]
Each item indicates how much of a production has been seen at a point in the process of parsing.
...
Finally we can look at the exact algorithm in LR(0) parsing:

\begin{itemize}
  \item Design the augmented grammar
  \item Construct a goto graph using closure and goto operations on LR(0) items
  \item Design the parsing table
  \item Execute parsing on the string
\end{itemize}
