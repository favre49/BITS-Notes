...
The \textbf{First set}, denoted by \texttt{FIRST(X)} for a grammar symbol $X$ is the set of tokens that begin the strings derivable form $X$.

To calculate the First Set, we use the following rules:

\begin{itemize}
  \item If $P$ is a non terminal and $P \rightarrow Q_1Q_2...Q_k$ is a production, then if for some $i$, First($Q_i$) = $\{x\}$ and $\epsilon$ is in all of First($Q_j$) (such that $j < i$), then First($P$) = $\{x\}$. If $\epsilon$ is in First($Q_i$) for all $i \in [1,k]$, then First($P$) = $\{\epsilon\}$
\end{itemize}

\textit{TODO: Understand and describe First Set better}
